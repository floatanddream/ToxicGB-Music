...
import { PlayIcon, ClockIcon, UserIcon, HeartIcon, PlusIcon } from 'lucide-vue-next';
...
<template>
  <div class="playlist-section px-4 md:px-6">
    <div class="section-header mb-8">
      <h2 class="text-3xl font-bold tracking-tight dark:text-white">推荐歌单</h2>
      <p class="text-gray-600 dark:text-gray-300 mt-2">发现属于你的音乐世界</p>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 playlist-grid">
      <Card
        ...
        :key="playlist.id"
        class="group hover:shadow-2xl transition-all duration-300 cursor-pointer dark:bg-gray-800/60 bg-white/70 backdrop-blur-md border border-white/20 dark:border-gray-700/30"
      >
        ...
        <CardContent class="p-4">
          <CardTitle class="text-base font-semibold mb-2 dark:text-white text-gray-800 line-clamp-1">{{ playlist.name }}</CardTitle>
          <p class="text-sm text-gray-600 dark:text-gray-300 mb-4 line-clamp-2 min-h-[2.5rem]">{{ playlist.description }}</p>
          ...
          <div class="flex items-center gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              class="w-8 h-8 p-0 border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700"
              @click.stop
            >
