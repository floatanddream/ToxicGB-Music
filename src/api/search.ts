...
export async function searchBySinger(keyword: string) {
  return await request.get(`/search?keywords=${keyword}&type=100&limit=100`)
}
export async function searchByPlaylist(keyword: string) {
  return await request.get(`/search?keywords=${keyword}&type=1000&limit=100`)
}
export async function searchByUser(keyword: string) {
  return await request.get(`/search?keywords=${keyword}&type=1002&limit=100`)
}
