...
export const transformAlbums = (
  rawAlbums: MusicTypes.RawAlbum,
  ...
  return {
    ...
    releaseDate: formatDate
      ? formatTimestampToDate(rawAlbums.publishTime)
      : String(rawAlbums.publishTime),
...
export const transformToPlaylist = (rawPlaylist: any): MusicTypes.Playlist => {
  ...
  return {
    ...
    playCount: formatNumber(rawPlaylist.playCount), // 格式化播放量
    likeCount: String(rawPlaylist.bookCount | rawPlaylist.subscribedCount | 0), // bookCount 对应收藏数
...
export const transformToSong = (rawData: any): MusicTypes.Song => {
  // 转换艺术家信息（从 ar 数组）
  const artists: MusicTypes.Artist[] =
    rawData.ar?.map((artist: any) => ({
      id: String(artist.id),
      name: artist.name,
